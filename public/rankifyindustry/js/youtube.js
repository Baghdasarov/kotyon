...
function init(){
  addFields(inputNum);
  document.getElementById("start").onclick = startSearch;
...
function beginUpdating(){
  ...
  for (var i=0; i<workData.length; i++){
    ...
    var t2 = obj['term2'];
    var vids = obj['vids'];
    setTimeout(checkTerm, timeout, vids, t1, t2);
    timeout += tdif;
  }
}

function checkTerm(vids, t1, t2){
  for (var j=0; j<vids.length; j++){
    var vid = vids[j];
    var index = j + 1;
    checkProcess(vid, t1, t2, index);
  }
}
...
function checkProcess(vid, t1, t2, indx){
  if (progress == 100 || !running) {
  	progress = 0;
  	running = false;
  	return;
  }
  var data = {"token" : token, "vid" : vid, "term1" : t1, "term2" : t2, "rank" : indx};
  $.ajax({
    method: "POST",
    url: "youtube.php",
    data: data,
    ...
    error: function(msg){
      ...
      if (msg.status != 503) setTimeout(checkProcess, 3000, vid, t1, t2, indx);
    },
    statusCode: {
    503: function() {
      //console.log("HTTP 503");
      setTimeout(checkProcess, 3000, vid, t1, t2, indx);
    }
  }
  });
}