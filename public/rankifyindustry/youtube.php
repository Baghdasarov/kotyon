...
$password = "@novelconceptdata2000";
$pass = "";
$loggenIn = false;

if (isset($_POST['pass'])) {
  $pass = $_POST['pass'];
  if ($pass == $password) $loggenIn = true;
...
if (!$loggenIn) {
  ...
  <body>
    <form style="width: 400px;position: relative;margin: 50px auto;" action="youtube.php" method="POST">
...
$items = array();
$max = $max_res;
if (isset($_POST['max_res_0'])) $max = $_POST['max_res_0'];
for ($i=1; $i<=50; $i++){
  //echo 'term_'.$i.": ".$_POST['term_'.$i];
  if (!isset($_POST['term_'.$i]) || !isset($_POST['sterm_'.$i])) continue;
  $item = array($_POST['term_'.$i], $_POST['sterm_'.$i], $max);
...
if (isset($_POST['lang']) && isset($langs[$_POST['lang']])) $lang = $langs[$_POST['lang']];
if (count($items) > 0 && !isset($_POST['token'])) exit();
...
foreach ($items as $key => $value){
  ...
  $term = urlencode($term);
  $sterm = $value[1];
  if (isset($value[2])) $max_res = $value[2];
  $t = array();
  $t['term1'] = $term;
  $t['term2'] = $sterm;
  $iter = 10;
  if ($max_res <= 50) $iter = $max_res;
  $nextPage = "null";
  $vids = array();
  for ($i=$iter; $i<=$max_res; $i+=$iter){
    ...
    if ($loc) $request .= "&location=".urlencode($loc)."&locationRadius=".$radius;
    if ($nextPage != "null") $request .= "&pageToken=".$nextPage;
    //echo $request."<br><br>";
    $res = requestAPI($request);
    $res = json_decode($res, true);
    $res = arrayCastRecursive($res);
    //var_dump($res);
    if (isset($res['nextPageToken'])) $nextPage = $res['nextPageToken'];
    $res = $res['items'];
    //var_dump($res);
...
if (isset($_POST['vid']) && isset($_POST['term1']) && isset($_POST['term2'])) {
  $vid = $_POST['vid'];
  $term = $_POST['term1'];
  $sterm = $_POST['term2'];
  ...
  $vres = arrayCastRecursive($vres);
  if (isset($vres['items'])) $vres = $vres['items'];
  else $vres = array();
  if (!isset($vres['snippet'])) $vres = $vres[0];
  ...
  $year = $year[0];
  $month = explode("-", $date);
  if (count($month) >= 2) $month = $month[1];
  else $month = $month[0];
  $day = explode("-", $date);
  if (count($day) >= 3) $day = $day[2];
  else $day = $day[1];
  ...
  //var_dump($sres['n']);
  if (isset($sres['lang']['url'])) $surl = $sres['lang']['url'];
  else $sres['lang'] = array('url' => '');
  if (isset($sres['lang']) && isset($sres['lang']['n'])){
    if ($sres['lang']['n'] != "en"){
      $surl = "";
      if (isset($sres['autotrans'])) $sres = $sres['autotrans'];
      else $sres = array();
  ...
  $cres = arrayCastRecursive($cres);
  $cres = $cres['items'];
  if (!isset($cres['snippet'])) $cres = $cres[0];
  ...
  if (strpos($hour, "S") !== false) $hour = 0;
  if (strpos($duration, "H") !== false) $duration = substr($duration, strpos($duration, "H")+1);
  //var_dump($duration);
  if (strpos($duration, "M") !== false) {
    $minute = explode("M", $duration);
    $minute = $minute[0];
  } else $minute = 0;
  if (strpos($duration, "M") !== false) $duration = substr($duration, strpos($duration, "M")+1);
  ...
  if (isset($stats['commentCount'])) $commCount = $stats['commentCount'];
  else $commCount = 0;
  $ind = "";
  if (isset($vres['snippet']['thumbnails']['maxres'])) $ind = "maxres";
  else if (isset($vres['snippet']['thumbnails']['standard'])) $ind = "standard";
  else if (isset($vres['snippet']['thumbnails']['high'])) $ind = "high";
  else if (isset($vres['snippet']['thumbnails']['medium'])) $ind = "medium";
  else if (isset($vres['snippet']['thumbnails']['default'])) $ind = "default";
  $maxres = $vres['snippet']['thumbnails'][$ind]['width']."x".$vres['snippet']['thumbnails'][$ind]['height'];
  $mpres = $vres['snippet']['thumbnails'][$ind]['width'];
  if ($def == "hd") {
    if ($mpres <= 720) $mpres = 720;
    else $mpres = 1080;
  } else {
    $mpres = 480;
  }
  ...
  if (!$sterm) $sterm2 = "null_string_second_term_check";
  if ($ctitle) {
    $t1icn = substr_count(strtolower($ctitle), $term);
    if ($t1icn > 0) $t1icn = "Yes";
    else $t1icn = "No";
    $t2icn = substr_count(strtolower($ctitle), $sterm2);
    if ($t2icn > 0) $t2icn = "Yes";
    else $t2icn = "No";
  } else {
    $t1icn = 0;
    $t2icn = 0;
  ...
  if (count($match) > 0) $match = $match[0];
  else $match = array();
  //var_dump($match);
  foreach ($match as $key => $value){
    preg_match('/content="(.*)"\>/', $value, $m);
    $match[$key] = $m[1];
    if ($term) if (strpos($m[1], $term) !== false) $t1it++;
    if ($sterm) if (strpos($m[1], $sterm) !== false) $t2it++;
  }
  ...
  */
  ...
  for ($i=0; $i<count($ct); $i++) {
    ...
    $rnk = explode($d, $value);
    //var_dump($rnk);
    if (isset($rnk[2])) $rnk = (int) $rnk[2];
    else $rnk = 0;
    $res = str_replace("\n", "", $res);
    $value = str_replace("\n", "", $value);
    if ($rank > $rnk && isset($ct[$i+1]) && $rank < (int) $ct[$i+1]) $value = $res."\n".$value;
    else if ($rank > $rnk && !isset($ct[$i+1])) $value = $value."\n".$res;
    $ncont .= $value."\n";
  }
  //var_dump($ct);
  //$content .= $res;
...
<div class="wrapper">
  ...
  <form method="POST" action="./youtube.php" class="form" id="main_form">
